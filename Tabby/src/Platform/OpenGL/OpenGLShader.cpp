...
#include "/usr/local/include/spirv_cross/spirv_cross.hpp"
#include "/usr/local/include/spirv_cross/spirv_glsl.hpp"
#include "/usr/local/include/shaderc/shaderc.hpp"
...
namespace Tabby {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			TB_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			std::string type = source.substr(begin, eol - begin);
			TB_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					TB_CORE_ERROR(module.GetErrorMessage());
					TB_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			TB_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		TB_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		TB_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		TB_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		TB_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			TB_CORE_TRACE("  {0}", resource.name);
			TB_CORE_TRACE("    Size = {0}", bufferSize);
			TB_CORE_TRACE("    Binding = {0}", binding);
			TB_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
}